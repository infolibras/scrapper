...
class DitechSpider(scrapy.Spider):
    ...
    def parse_term(self, response):
        ...
        termo = response.css("h1.header-post-title-class::text").extract_first().strip().capitalize()
        ...
        definicao_infos = response.css(".entry-content p::text").extract_first().strip().split(": ")
        ...
        if len(definicao_infos) > 1:
            ...
            if correspondencia is not None:
                ...
                if termo_info != definicao["termo"] and (not tem_parenteses or termo_info != definicao["variacoes"][0]["variacao"]):
                    definicao["variacoes"] = [{ "variacao": termo_info, "explicacao": "" }]

                variacao = correspondencia.group(2).strip()[1:-1].capitalize()

                if variacao != definicao["termo"] and (not tem_parenteses or variacao != definicao["variacoes"][0]["variacao"]):
                    definicao["variacoes"] = [{ "variacao": variacao, "explicacao": "" }]

            else:
                if termo_info != definicao["termo"] and (not tem_parenteses or termo_info != definicao["variacoes"][0]["variacao"]):
                    definicao["variacoes"] = [{ "variacao": definicao_infos[0].strip().capitalize(), "explicacao": "" }]
